...
if exist(b.finalAnat, 'file') == 2
    %sprintf('anatomical already preprocessed, running functional only...');
    %% expand frames
    ...
    matlabbatch{3}.spm.spatial.realign.estwrite.roptions.prefix = 'r';
     %% coregistration
    matlabbatch{7}.spm.spatial.coreg.estimate.ref(1) = b.anatSkullStrippedNative;
    ...
    matlabbatch{8}.spm.util.defs.comp{1}.def(1) = b.deformationField  ;
    ...
    matlabbatch{8}.spm.util.defs.out{1}.pull.prefix = '';
else
    %% expand frames
    matlabbatch{1}.spm.util.exp_frames.files = b.funcFullpath;
    matlabbatch{1}.spm.util.exp_frames.frames = Inf;
    ...
    %% image calculator - combine GM,WM,CSF masks into brain mask
    ...
    %% image calculator - apply brain mask to anatomical T1 image for skull stripping
    ...
    matlabbatch{9}.spm.util.defs.out{1}.pull.prefix = '';
    %% QA
    % matlabbatch{10}.localhelp.qa.QArunall.orig_anat = b.anatFullpath;
    % matlabbatch{10}.localhelp.qa.QArunall.orig_fmri = b.funcFullpath;
    % matlabbatch{10}.localhelp.qa.QArunall.final_fmri(1) = cfg_dep('Deformations: Warped Images', substruct('.','val', '{}',{8}, '.','val', '{}',{1}, '.','val', '{}',{1}), substruct('.','warped'));
    % matlabbatch{10}.localhelp.qa.QArunall.final_anat(1) = cfg_dep('Deformations: Warped Images', substruct('.','val', '{}',{9}, '.','val', '{}',{1}, '.','val', '{}',{1}), substruct('.','warped'));
    % matlabbatch{10}.localhelp.qa.QArunall.mot_params(1) = cfg_dep('Realign: Estimate & Reslice: Realignment Param File (Sess 1)', substruct('.','val', '{}',{3}, '.','val', '{}',{1}, '.','val', '{}',{1}, '.','val', '{}',{1}), substruct('.','sess', '()',{1}, '.','rpfile'));
 %% run batch
% spm('defaults','fmri')
% spm_jobman('initcfg')
